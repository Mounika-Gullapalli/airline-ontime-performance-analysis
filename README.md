...
The raw CSV is included in this repo as `data/flights.zip` (compressed from 58MB to 4.8MB)
— unzip it to `data/flights.csv` before running the notebook or script. Alternatively,
follow the download steps above to pull a fresh copy directly from BTS.
...
## How to Reproduce
```bash
pip install pandas scipy matplotlib seaborn jupyter --break-system-packages
```
Unzip `data/flights.zip` to `data/flights.csv` (or follow the download steps above to pull
a fresh copy directly from BTS), then open and run `airline_delay_analysis.ipynb` in
Jupyter Notebook or JupyterLab. All charts and statistical outputs are also already saved
inside the notebook, so no re-run is required to view results.
